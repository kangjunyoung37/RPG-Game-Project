    ...
    public class LeaderboardController : MonoBehaviour
    {
        ...
        public void GetUserScore(string userId)
        {
            ...
            databaseRef.Child(AllScoreDataPath).OrderByChild(UserScore.userIdPath).StartAt(userId).EndAt(userId).GetValueAsync().ContinueWith(task =>
            {
                ...
                {
                    var scores = ParseVailidUserScoreRecords(task.Result, -1, -1).ToList();
                    if(scores.Count == 0)
                    {
                        retrievedScroreArgs = new UserScoreArgs(null, string.Format("No Scores for User{0} whithin time range ({1}-{2})", userId, -1, -1));
                    ...
                    else
                    {
                        ...
                        retrievedScroreArgs = new UserScoreArgs(userScore, userScore.userId + "Retrieved");
                    }
                }
        ...
        private void GetIntialTopScores(long batchEnd)
        {
            ...
            query.GetValueAsync().ContinueWith(task =>
            {
                if(task.Exception != null)
                {
                    ...
                    Debug.LogWarning("Don't get TopScores because" + task.Exception);
                    return;
                }
                ...
                var scores = ParseVailidUserScoreRecords(task.Result, -1, -1);
